...
# โหลดไฟล์ .kv เพื่อกำหนด UI
Builder.load_file('asdp.kv')

# ส่วนของแถบแสดงผลความน่าจะเป็นของแต่ละโรค
class PredictionBar(BoxLayout):
    disease_name = StringProperty('') # ชื่อของโรค
    probability = ObjectProperty(0) # ค่าความน่าจะเป็นของโรค
    bar_color = ObjectProperty([0, 0, 0, 1]) # สีของแถบที่แสดง

# คลาสหลักที่ใช้แสดงหน้าหลักของแอปพลิเคชัน
class MainScreen(BoxLayout):
    selected_image_path = StringProperty('') # เก็บเส้นทางของภาพที่เลือก
    model = None # โมเดลที่ใช้สำหรับพยากรณ์
    classes = ['bacterial', 'fungal', 'healthy', 'hypersensitivity'] # รายชื่อโรคที่สามารถพยากรณ์ได้
    
    # สีสำหรับแสดงผลแต่ละประเภทของโรค
    colors = {
    ...
    def __init__(self, **kwargs):
        ...
        self.ensure_uploaded_images_folder() # ตรวจสอบและสร้างโฟลเดอร์สำหรับเก็บรูป
        self.load_model() # โหลดโมเดล 
    
    def ensure_uploaded_images_folder(self):
        # ตรวจสอบและสร้างโฟลเดอร์สำหรับบันทึกรูปที่อัปโหลด
    ...
    def process_selected_file(self, file_path):
        # คัดลอกไฟล์ไปยังโฟลเดอร์และอัปเดตรูปแสดงตัวอย่าง
    ...
    def process_image(self, dt):
        # โหลดภาพ ประมวลผล และพยากรณ์โรค
        try:
